...
joinToLength l ss = [s1 ++ s2 | s1 <- ss, s2 <- ss, length (s1 ++ s2) == l]
...
sumRights = sum . fmap (fromRight 0)
...
multiApp :: (Functor f) => (f b1 -> b2) -> f (p -> b1) -> p -> b2
multiApp f gs xs = f $ fmap ($ xs) gs
...
-- interpreter [] = []
-- interpreter :: [String] -> ([Int], Int, Int)
interpreter :: [String] -> [String]
interpreter xs =
  map show $
    fst $
