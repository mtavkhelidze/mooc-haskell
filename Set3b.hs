...
buildList' xs s 0 = xs
buildList' ss s n = buildList' (s : ss) s (n - 1)

buildList :: Int -> Int -> Int -> [Int]
buildList s n e = buildList' [e] s n
...
sumRange (x : xs) sum = sumRange xs (sum + x)
sumRange [] sum = sum

sums :: Int -> [Int]
sums i = [sumRange [1 .. x] 0 | x <- [1 .. i]]
...
indexDefault (h : t) i def =
  if i == 0
    then h
    else indexDefault t (i - 1) def
...
mymaximum biggerThan initial (x : xs)
  | biggerThan x initial = mymaximum biggerThan x xs
  | otherwise = mymaximum biggerThan initial xs
