...
data Vehicle = Bike | Bus | Tram | Train deriving (Eq, Show)
...
data BusTicket = SingleTicket | MonthlyTicket String deriving (Eq, Show)
...
buyOneMore (MkShoppingEntry name price count) = MkShoppingEntry name price (succ count)
...
data Person = PersonWithName String Int
  deriving (Show)

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = PersonWithName "Fred" 90

-- getName returns the name of the person
getName :: Person -> String
getName (PersonWithName name _) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (PersonWithName _ age) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (PersonWithName _ age) = PersonWithName name age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (PersonWithName name _) = PersonWithName name age
...
-- @misha: botth ways are fine same performance-wise
...
data UpDown = TickUp Int | TickDown Int deriving (Show)
...
toNat z =
  if z < 0
    then Nothing
    else Just $ iterate PlusOne Zero !! z
